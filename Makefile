...
ifeq ($(DISTRO), fedora)
	...
	OS_VERSION = 9.4
        ...
        EXTRA_LABELS := $(EXTRA_LABELS) --label=io.k8s.description="Bootc for AMD ROCm provides a container runtime for AMD ROCm accelerated workloads"
        EXTRA_LABELS := $(EXTRA_LABELS) --label=io.k8s.display-name="Bootc for AMD ROCm"
...
IMAGE_NAME ?= ${DISTRO}-bootc-amd-rocm
IMAGE_TAG ?= $(OS_VERSION)-$(DRIVER_VERSION)
...
build-container:
	"${CONTAINER_TOOL}" build \
		...
		--label org.opencontainers.image.version=${OS_VERSION}-${DRIVER_VERSION} \
		--label vcs-ref=${VCS_REF} \
		--label version=${OS_VERSION}-${DRIVER_VERSION} \
		--cap-add SYS_ADMIN \
		--file Containerfile \
